...
if __name__ == '__main__':
    ...
    # foldername = '2011_09_26_drive_0014_sync'
    ...
    with torch.no_grad():
        for sample_idx in range(len(demo_dataset)):
            ...
            bev_map = (bev_map.permute(1, 2, 0).numpy() * 255).astype(np.uint8) # chw变为hwc
            bev_map = cv2.resize(bev_map, (cnf.BEV_WIDTH, cnf.BEV_HEIGHT))
            bev_map = draw_predictions(bev_map, detections, configs.num_classes)

            # Rotate the bev_map
            bev_map = cv2.rotate(bev_map, cv2.ROTATE_180)
            cv2.imshow("front",bev_map)
            cv2.waitKey(0)
            pdb.set_trace()
            ...
            if out_cap is None:
                ...
                fourcc = cv2.VideoWriter_fourcc(*'MJPG') # fourcc意为四字符代码（Four-Character Codes）编码器
                ...
                out_cap = cv2.VideoWriter(out_path, fourcc, 30, (out_cap_w, out_cap_h)) # 30 = FPS
