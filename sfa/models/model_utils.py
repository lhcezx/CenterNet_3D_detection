...
# 创建网络模型，定义backbone和层数
def create_model(configs, pointpillars):
    ...
    if 'fpn_resnet' in configs.arch:
        ...
        model = fpn_resnet.get_pose_net(num_layers=num_layers, heads=configs.heads, head_conv=configs.head_conv,
                                        imagenet_pretrained=configs.imagenet_pretrained, PP = pointpillars)
...
def get_num_parameters(model):
    ...
    if hasattr(model, 'module'):
        num_parameters = sum(p.numel() for p in model.module.parameters() if p.requires_grad) # 返回parameter的数量
    else:
