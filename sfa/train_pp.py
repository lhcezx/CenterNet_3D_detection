...
from data_process.kitti_dataloader import create_train_dataloader, create_train_dataloader_voxel, create_val_dataloader, create_val_dataloader_voxel
...
def main_worker(gpu_idx, configs):
    ...
    # model
    model = create_model(configs, True)
    ...
    # Create dataloader 
    train_dataloader, train_sampler = create_train_dataloader_voxel(configs)
    if logger is not None:
        logger.info('number of batches in training set: {}'.format(len(train_dataloader)))

    if configs.evaluate:
        val_dataloader = create_val_dataloader_voxel(configs)
...
def train_one_epoch(train_dataloader, model, optimizer, lr_scheduler, epoch, configs, logger, tb_writer):
    ...
    for batch_idx, batch_data in enumerate(tqdm(train_dataloader)):
        ...
        metadatas, input, targets = batch_data
        batch_size = input["voxels"].size(0)
        global_step = num_iters_per_epoch * (epoch - 1) + batch_idx + 1
        for k in targets.keys():
            targets[k] = targets[k].to(configs.device, non_blocking=True)
        for k in input.keys():
            input[k] = input[k].to(configs.device, non_blocking=True)
        outputs = model(input)
