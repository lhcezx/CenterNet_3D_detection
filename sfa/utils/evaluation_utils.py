...
def _gather_feat(feat, ind, mask=None):
    dim = feat.size(2)
    # unsqueeze(2): 在维度2上扩展一个维度, expand(ind.size(0), ind.size(1), dim)
    # expand只能对维度为一的维度进行扩张
    ind = ind.unsqueeze(2).expand(ind.size(0), ind.size(1), dim)
    # (batch, cat*K, 1) -> # (batch, K, 1) 在cat*K(dim = 1)中根据下标ind找到其对应的      
...
def _topk(scores, K=40):
    ...
    # (batch, cat, K) 将宽高展开成一维并求出每个类cat中前K个置信度最高的点并求出其下标ind(2200,5600...)，此时的ind被约束在(height * width)范围内，因为每个类只有(height * width)个点
    topk_scores, topk_inds = torch.topk(scores.view(batch, cat, -1), K) # ([1, 3, 152, 152]) -> ([1, 3, 50])
    topk_inds = topk_inds % (height * width) # 防止inds越界
    
    # 通过ind很容易可以计算出x,y坐标
    topk_ys = (torch.floor_divide(topk_inds, width)).float()
    topk_xs = (topk_inds % width).int().float()
    # (batch, K) 此时对整个batch进行topk, 由于每个类有K个值，并且每个类的K个值依次排开（第一个类的K个值，第二个类的K个值一直到最后一个类的K个值一次排开），此时求出来的ind%K可以直接获得类别
    topk_score, topk_ind = torch.topk(topk_scores.view(batch, -1), K)
    # 例如(K = 40, 一共有5个类，200个样本，如果我们取一个下标为77的值，那么这个值的位置就在第一个类的第37这个位置上)
    topk_clses = (torch.floor_divide(topk_ind, K)).int()
    # topk_s, topk_i = torch.topk(scores.view(batch, -1), K)
    # cls = (torch.floor_divide(topk_i, height*width)).int()
    # print(cls)
    # 找到Batch中每张图片的前K个inds, ys, xs
    # 在每个类的前K个最值(inds)的基础上在batch中每个图片找前k个最值(ind)
...
# 解码器，通过hm, off, direction, z_coor, dim输出detection的信息
def decode(hm_cen, cen_offset, direction, z_coor, dim, K=40):
    ...
    #     for j in range(hm_cen.shape[1]):
    #         cv2.imshow("hm",hm_cen[i,j])
    #         cv2.waitKey(0)
    # pdb.set_trace()
    hm_cen = _nms(hm_cen)
    ...
    return detections   # (batch, K, 10)


def get_yaw(direction):
    return np.arctan2(direction[:, 0:1], direction[:, 1:2])

# detection后处理，ret列表包含了batch中每一张图片的top_pred字典，top_pred字典包含了经过置信度筛选过后的图片中每个类下的对应物体信息
def post_processing(detections, num_classes=3, down_ratio=4, peak_thresh=0.2):
    ...
    for i in range(detections.shape[0]):
        ...
        for j in range(num_classes):
            # 每一个类和K个目标的类别进行判别，每进行一次循环就能获取类j的(K,)列表，列表中的K个样本值等于1的样本属于该类，0的话则不属于该类
            inds = (classes == j)  # classes是一个包含了一张图片中的前K个置信度最高的物体的类别，通过和j做==运算来判断这K个样本中j类别的物体就是True
            # x, y, z, h, w, l, yaw
            # 对i图片的j类别取每一个ind数组里面的为True的位置的数据
            # 对xs, ys上采样回到rgb图像的尺度，对尺寸hwl返回到BEV图下的尺度
            # top_pred这个列表就包括了一张图片中每个类别下对应的样本信息，同时也通过j循环讲每个类别分开。
            top_preds[j] = np.concatenate([
                ...
                get_yaw(detections[i, inds, 7:9]).astype(np.float32)], axis=1) # 偏航角
