...
def create_lr_scheduler(optimizer, configs):
    ...
    if configs.lr_type == 'multi_step':
        ...
        lr_scheduler = LambdaLR(optimizer, multi_step_scheduler) # LambdaLR(optimizer, lr_lambda, last_epoch) 其中lr_lambda需要传入一个根据epoch变换的函数或者一个list

    elif configs.lr_type == 'cosin':
        # Scheduler https://arxiv.org/pdf/1812.01187.pdf
        lf = lambda x: (((1 + math.cos(x * math.pi / configs.num_epochs)) / 2) ** 1.0) * 0.9 + 0.1  # cosin
...
def save_checkpoint(checkpoints_dir, saved_fn, model_state_dict, utils_state_dict, epoch, is_best = False):
    """Save checkpoint every epoch only is best model or after every checkpoint_freq epoch"""
    if is_best:
        model_save_path = os.path.join(checkpoints_dir, 'Model_best.pth')
        utils_save_path = os.path.join(checkpoints_dir, 'Utils_best.pth')
    else:
