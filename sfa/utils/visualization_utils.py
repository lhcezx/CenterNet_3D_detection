...
def compute_box_3d(dim, location, ry):
    ...
    # ry: 1
    # return: 8 x 3
    R = roty(ry) # 构造围绕y轴的旋转矩阵
    h, w, l = dim
    x_corners = [l / 2, l / 2, -l / 2, -l / 2, l / 2, l / 2, -l / 2, -l / 2]
    y_corners = [0, 0, 0, 0, -h, -h, -h, -h]
    z_corners = [w / 2, -w / 2, -w / 2, w / 2, w / 2, -w / 2, -w / 2, w / 2]
    corners = np.array([x_corners, y_corners, z_corners], dtype=np.float32)
    corners_3d = np.dot(R, corners) # 绕y轴旋转
    # 中心点坐标加上长宽高获获得3dbbox
...
def draw_box_3d(image, corners, color=(0, 0, 255)):
    ...
    for ind_f in range(3, -1, -1):
        ...
        # 如果是正面的话，车辆的朝向面，连接对角线上的点用于表示这是正面       
        if ind_f == 0:
...
def show_rgb_image_with_boxes(img, labels, calib):
    for box_idx, label in enumerate(labels):
        cls_id, location, dim, ry = label[0], label[1:4], label[4:7], label[7] # 类别，三维物体的位置x,y,z，3D的长宽高，y轴偏向角
...
# 把RGB图像和BEV图像结合到一起最后输出demo的图像
def merge_rgb_to_bev(img_rgb, img_bev, output_width):
    # 获取RGB图像尺寸和ratio, resize到合适的比例
    ...
    # 获取BEV图像尺寸和ratio然后进行resize到和是比例
